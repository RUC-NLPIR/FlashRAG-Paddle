...
class Index_Builder:
    ...
    def __init__(self, retrieval_method, model_path, corpus_path, save_dir,
        max_length, batch_size, use_fp16, pooling_method, faiss_type=None,
        embedding_path=None, save_embedding=False, faiss_gpu=False,
        use_sentence_transformer=False, bm25_backend='bm25s'):
        ...
        if not os.path.exists(self.save_dir):
            ...
            warnings.warn(
                ...
                , UserWarning)
        self.index_save_path = os.path.join(self.save_dir,
            f'{self.retrieval_method}_{self.faiss_type}.index')
        self.embedding_save_path = os.path.join(self.save_dir,
    ...
    def build_bm25_index_pyserini(self):
        ...
        pyserini_args = ['--collection', 'JsonCollection', '--input',
            temp_dir, '--index', self.save_dir, '--generator',
            'DefaultLuceneDocumentGenerator', '--threads', '1']
    ...
    def build_bm25_index_bm25s(self):
        ...
        self.save_dir = os.path.join(self.save_dir, 'bm25')
        os.makedirs(self.save_dir, exist_ok=True)
        corpus = datasets.load_dataset('json', data_files=self.corpus_path,
            split='train')
    ...
    def _load_embedding(self, embedding_path, corpus_size, hidden_size):
        all_embeddings = np.memmap(embedding_path, mode='r', dtype=np.float32
            ).reshape(corpus_size, hidden_size)
        return all_embeddings

    def _save_embedding(self, all_embeddings):
        memmap = np.memmap(self.embedding_save_path, shape=tuple(
            all_embeddings.shape), mode='w+', dtype=all_embeddings.dtype)
        length = tuple(all_embeddings.shape)[0]
        save_batch_size = 10000
        if length > save_batch_size:
            for i in tqdm(range(0, length, save_batch_size), leave=False,
    ...
    def st_encode_all(self):
        ...
        all_embeddings = self.encoder.encode(sentence_list, batch_size=self
            .batch_size)
        return all_embeddings
    ...
    def encode_all(self):
        ...
        for start_idx in tqdm(range(0, len(self.corpus), self.batch_size),
            ...
            # print(batch_data)
            inputs = self.tokenizer(batch_data, padding=True, truncation=True, return_tensors='pd', max_length=self.max_length)
            inputs = {k: v.cuda(blocking=True) for k, v in inputs.items()}
            # print(inputs)
            if 'T5' in type(self.encoder).__name__:
                decoder_input_ids = paddle.zeros(shape=(tuple(inputs[
                    "input_ids"].shape)[0], 1), dtype='int64').to(inputs[
                    'input_ids'].place)
                output = self.encoder(**inputs, decoder_input_ids=
                ...
                embeddings = pooling(output.pooler_output, output.
                    last_hidden_state, inputs['attention_mask'], self.
                    pooling_method)
                if 'dpr' not in self.retrieval_method:
                    embeddings = paddle.nn.functional.normalize(x=
                        embeddings, axis=-1)
    ...
    def build_dense_index(self):
        ...
        if self.use_sentence_transformer:
            ...
            self.encoder, self.tokenizer = load_model(model_path=self.
                model_path, use_fp16=self.use_fp16)
        ...
        if self.embedding_path is not None:
            ...
            all_embeddings = self.st_encode_all(
                ) if self.use_sentence_transformer else self.encode_all()
            if self.save_embedding:
                self._save_embedding(all_embeddings)
            del self.corpus
        print('Creating index')
        dim = tuple(all_embeddings.shape)[-1]
        faiss_index = faiss.index_factory(dim, self.faiss_type, faiss.
...
MODEL2POOLING = {'e5': 'mean', 'bge': 'cls', 'contriever': 'mean', 'jina':
    'mean'}
...
def main():
    ...
    parser.add_argument('--sentence_transformer', action='store_true',
        default=False)
    parser.add_argument('--bm25_backend', default='bm25s', choices=['bm25s',
        'pyserini'])
    ...
    index_builder = Index_Builder(retrieval_method=args.retrieval_method,
        model_path=args.model_path, corpus_path=args.corpus_path, save_dir=
        args.save_dir, max_length=args.max_length, batch_size=args.
        batch_size, use_fp16=args.use_fp16, pooling_method=pooling_method,
        faiss_type=args.faiss_type, embedding_path=args.embedding_path,
        save_embedding=args.save_embedding, faiss_gpu=args.faiss_gpu,
        use_sentence_transformer=args.sentence_transformer, bm25_backend=
        args.bm25_backend)
