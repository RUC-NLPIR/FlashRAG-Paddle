...
class BaseReranker:
    ...
    def get_rerank_scores(self, query_list: List[str], doc_list: List[str],
        batch_size):
    ...
    def rerank(self, query_list, doc_list, batch_size=None, topk=None):
        ...
        all_scores = self.get_rerank_scores(query_list, doc_contents,
            batch_size)
...
class CrossReranker(BaseReranker):
    ...
    def __init__(self, config):
        ...
        self.tokenizer = transformers.AutoTokenizer.from_pretrained(self.
            reranker_model_path)
        self.ranker = (transformers.AutoModelForSequenceClassification.
            from_pretrained(self.reranker_model_path, num_labels=1))
        self.ranker.eval()
    ...
    def get_rerank_scores(self, query_list, doc_list, batch_size):
        ...
        all_scores = []
        for start_idx in tqdm(range(0, len(all_pairs), batch_size), desc=
            'Reranking process: '):
            pair_batch = all_pairs[start_idx:start_idx + batch_size]
            inputs = self.tokenizer(pair_batch, padding=True, truncation=
                True, return_tensors='pd', max_length=self.max_length)
            batch_scores = self.ranker(**inputs, return_dict=True).logits.view(
                -1).astype(dtype='float32').cpu()
...
class BiReranker(BaseReranker):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.encoder = Encoder(model_name=self.reranker_model_name,
            model_path=self.reranker_model_path, pooling_method=config[
            'rerank_pooling_method'], max_length=self.max_length, use_fp16=
            config['rerank_use_fp16'])
