...
def cache_manager(func):
    ...
    def wrapper(self, query_list, num=None, return_score=False):
        ...
        if self.use_cache:
            ...
            if no_cache_query != []:
                no_cache_results, no_cache_scores = (self.
                    _batch_search_with_rerank(no_cache_query, num, True))
                no_cache_idx = 0
                for idx, res in enumerate(cache_results):
                    if res is None:
                        assert new_query_list[idx] == no_cache_query[
                            no_cache_idx]
                        cache_results = no_cache_results[no_cache_idx
                            ], no_cache_scores[no_cache_scores]
                        no_cache_idx += 1
            results, scores = [t[0] for t in cache_results], [t[1] for t in
                cache_results]
        else:
        ...
        if self.save_cache:
            ...
            for query, doc_items, doc_scores in zip(query_list,
...
class BaseRetriever:
    ...
    def __init__(self, config):
        ...
        if self.save_cache:
            self.cache_save_path = os.path.join(config['save_dir'],
    ...
    def _search(self, query: str, num: int, return_score: bool) ->List[Dict
        [str, str]]:
...
class BM25Retriever(BaseRetriever):
    ...
    def __init__(self, config):
        ...
        if self.backend == 'pyserini':
            ...
            self.searcher = bm25s.BM25.load(self.index_path, mmap=True,
                load_corpus=True)
    ...
    def _search(self, query: str, num: int=None, return_score=False) ->List[
        Dict[str, str]]:
        if num is None:
            num = self.topk
        if self.backend == 'pyserini':
            ...
            if len(hits) < num:
                ...
                hits = hits[:num]
            if self.contain_doc:
                all_contents = [json.loads(self.searcher.doc(hit.docid).raw
                    ())['contents'] for hit in hits]
...
class DenseRetriever(BaseRetriever):
    ...
    def _batch_search(self, query_list: List[str], num: int=None,
        ...
        results = []
        scores = []
        for start_idx in tqdm(range(0, len(query_list), batch_size), desc=
            'Retrieval process: '):
            ...
            batch_results = [batch_results[i * num:(i + 1) * num] for i in
                range(len(batch_idxs))]
